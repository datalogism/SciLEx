...
dict_abstract_data={}
for k in dict_meta_data.keys():
    if("abstract" in dict_meta_data[k]["message"].keys()):
        print("YEAH")
        dict_abstract_data[k]=dict_meta_data[k]["message"]["abstract"].replace("</jats:p>","").replace("<jats:p>","")
...
for p in all_data:
    z_id=p["key"]
    if z_id not in dict_abstract_data.keys():
        ...
        if(prefix_pwc_url in pwc_url):            
            ...
            PREFIX dcterms: <http://purl.org/dc/terms/>
                                 select ?id ?abstract where {
                                  <'''+lp_id+'''> dcterms:abstract ?abstract
                                 }
             ''')
            ret = sparql.query()
            d = ret.convert()
...
for z_id in dict_abstract_data.keys():
