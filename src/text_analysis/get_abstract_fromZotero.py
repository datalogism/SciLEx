            ...
            if(r_items.status_code ==  200 ):
                ...
                for paper in dict_papers[key]["items"]:
                    if("DOI" in paper["data"].keys()):
                        if(paper["data"]["DOI"]!="" and paper["data"]["DOI"]!="nan" and "/" in paper["data"]["DOI"]):
                            if("extra" in  paper["data"].keys() and paper["data"]["extra"]!="" and "['" in paper["data"]["extra"]):
                                try:
...
dict_export={}
for id_ in  list_id_interest.keys():
    dict_export[list_id_interest[id_]]=[]
    
for k in dict_papers.keys():
    lib=dict_papers[k]
    print(lib["key"])
    if lib["key"] in list_id_interest.keys():
        ...
        for paper in lib["items"]:
            ...
            if ("abstractNote" in paper["data"].keys()):
...
for coll in dict_export.keys():
   print(">>>>>>>>>>>>>>>",coll)
   with open(collect_dir+"/"+coll+"_abstract_iramuteq.txt", 'w') as file:
       for paper in dict_export[coll]:
           f_all_t.write("**** *"+coll+" *"+str(paper["year"])+" *"+paper["label"]+"\n")
           f_all_t.write(paper["title"]+"\n")
           f_all_a.write("**** *"+coll+" *"+str(paper["year"])+" *"+paper["label"]+"\n")
