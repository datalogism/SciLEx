...
if getDataChanges==True:
    ...
    val_count={}
    tag_changes={}
    for item in dict_papers["IJAAEH8Q"]["items"]:# DHHJ7YZF for models only / R67UC3PV for dataset / IJAAEH8Q for surveys
        z_id=item["key"]
        data=item["data"]
        tags=data["tags"]
        if(len(tags)>0):
            ...
            for t in tags:
                tag=t["tag"]
                if(tag in dict_tags_replace.keys()):
                          ...
                          for col in columns:
                              if(col not in ["delete","nb","replace_by","keep"]):
                                  ...
                                  if(str(val)!="nan"):
                                      ...
                                      if(col2 not in val_count.keys()):
                                          val_count[col2]=0
                                      if(col2 not in tags_new.keys()):
                                          tags_new[col2]=[]
