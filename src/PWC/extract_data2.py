...
def PaperWithCodetoZoteroFormat(row):
    ...
    #bookSection?
    zotero_temp={"title":"NA","publisher":"NA","itemType":"NA","authors":"NA","language":"NA","abstract":"NA","archiveID":"NA","archive":"NA","date":"NA","DOI":"","url":"NA","rights":"NA","pages":"NA","journalAbbreviation":"NA","volume":"NA","serie":"NA","issue":"NA"}
...
def getSubCollectionId(parent_coll,subcoll_name):
    ...
    while current_col_key is None :
        ...
        if(r_collections.status_code ==  200 ):
            ...
            found=False
            for d in data_collections:
                if(d["data"]["parentCollection"] == str(id_lib) and  d["data"]["name"]==subcoll_name):
            ...
            if current_col_key is None :
                ...
                body=json.dumps([{ "name" : subcoll_name, "parentCollection" : str(id_lib) }])
...
def collectTags(row):
    tags=[]
    if("methods" in row.keys() and len(row["methods"])>0):
        for m in row["methods"]:
            tag_name="Pwc_method:"+m["name"]
            tags.append({ "tag" :tag_name})
    if("tasks" in row.keys() and len(row["tasks"])>0):
        for t in row["tasks"]:
            tag_name="Pwc_task:"+t
            tags.append({ "tag" :tag_name})
    return tags
...
if getData:
    ...
    files=["dataset","methods","evaluation-tables","links-between-papers-and-code","papers-with-abstracts"]
    data={}
    for f in files:
        print(f)
    ...
    for row in data["dataset"]:
        for i in range(len(row["tasks"])):
            temp=row
            if(row["tasks"][i]["task"] not in dict_tasks_ds.keys()):
...
if cleanData:
    ...
    focused_tasks=["Relation Extraction",
    ...
    tasks_data_={}
    keep_ds_k=  ['name', 'full_name', 'homepage', 'description', 'paper', 'introduced_date', 'warning', 'modalities', 'languages', 'variants', 'num_papers']
    ...
    for row in data["dataset"]:
        ...
        for i in range(len(row["tasks"])):
            if( row["tasks"][i]["task"].lower() in focused_tasks):
                ...
                if(row["tasks"][i]["task"] not in tasks_data_.keys()):
                    tasks_data_[row["tasks"][i]["task"]]={"url":row["tasks"][i]["url"],"datasets":[row["url"]]}
                else:
    ...
    for row in data["evaluation-tables"]:
        if(row["task"].lower() in  focused_tasks):
            if(row["task"] not in task_eval_data_.keys()):
    ...
    for row in data["papers-with-abstracts"]:
        ...
        for task in row["tasks"]:
            if(task.lower() in focused_tasks):
                focused=True
                if(task in task_eval_data_.keys()):
                    for k in task_eval_data_[task].keys():
        ...
        if(focused):
            ...
            if(row["paper_url"] in list(dataset_data_papers.values())): #IS IT A DATASET PAPER ?
                datasets_papers_data[row["paper_url"]]=models_data_[row["paper_url"]]
            if( row["proceeding"] != None):
                if(row["proceeding"] not in proceeding_list.keys()):
            ...
            if(len(row["methods"])>0):
                for meth in row["methods"]:
                    if(meth["name"] not in methods_used.keys()):
...
if computeStat==True:
    ...
    for current in data["evaluation-tables"]:
        ...
        for ds in current["datasets"]:
            for rows in ds["sota"]["rows"]:
                ...
                if(rows["paper_url"] in sourcetopwc_url.keys()):
                    url2=sourcetopwc_url[rows["paper_url"]]
                    if(url2 in models_data_.keys() and url2 not in benchmak_links):
                        benchmak_links.append(url2)
                        
                        print("ADD THERE")
        if("subtasks" in current.keys() and len(current["subtasks"])>0):
            for st in current["subtasks"]:
                for ds2 in st["datasets"]:
                    for rows2 in ds2["sota"]["rows"]:
                        if(rows2["paper_url"] in sourcetopwc_url.keys()):
    ...
    for current in data["links-between-papers-and-code"]:
        ...
        if(current["paper_url"] in models_data_.keys()):
            if(current["paper_url"] not in code_links):
                print(current["paper_url"])
                code_links.append(current["paper_url"])
                nb_code +=1
                
    years=list(set([models_data_[k]["date"][0:4] for k in models_data_.keys() if "date" in models_data_[k].keys()]))
    sorted_years = sorted(years)
    variables=["Total", "Nb Preprint", "Nb code", "Nb benchmarked"]
    dict_years={"Total":[0]*len(sorted_years), "Nb Preprint":[0]*len(sorted_years), "Nb code":[0]*len(sorted_years), "Nb benchmarked":[0]*len(sorted_years)}
...
if pushData==True :
    ...
    libs="/collections/"
    for k in models_data_.keys():
        if("arxiv" not in models_data_[k]["url_abs"] or models_data_[k]['paper_url'] in code_links or models_data_[k]['paper_url'] in benchmak_links):
