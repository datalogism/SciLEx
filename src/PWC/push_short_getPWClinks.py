...
if getRefData==True:
    ...
    files=["dataset","evaluation-tables","links-between-papers-and-code","papers-with-abstracts"]
    ...
    for ds in data["dataset"]:
        ...
        if inscope:
            ...
            title="na"
            pwc_url=ds["url"]
            if("paper" in ds.keys() and ds["paper"] is not None):
            ...
            datasets_url[pwc_url]={"name":ds["name"],"title":title,"extra_url":extra_url}
            if(title!="na"):
    ...
    for paper in data["papers-with-abstracts"]:
        ...
        if inscope:
            ...
            paper_url[pwc_url]={"title":title,"extraurl":extra_url,"code":"na","results":{},"shortname":[]}
            paper_extra_url[extra_url]=pwc_url
            paper_title_url[title]=pwc_url
    
    for paper in data["links-between-papers-and-code"]:
        if(paper["paper_url"] in paper_url.keys()):
              ...
              for item in dict_papers[key]["items"]:
                  if(item["data"]["itemType"] not in ["attachment","webpage","note"]):
                      ...
                      if("paperwithcode" in archive):
                          if(archive in paper_url.keys()):
                              
                              print(">>> FOUND")
                              toADD={}
                              if(len(paper_url[archive]["shortname"])>0):
                                  toADD["shortTitle"]=", ".join(list(set(paper_url[archive]["shortname"])))
                              ...
                              if(paper_url[archive]["code"]!="" and (item["data"]["archiveLocation"] in ["","na","NA"]) ):
                                  toADD["archiveLocation"]=paper_url[archive]["code"]
                              else:
                                  print("CODE >",item["data"]["archiveLocation"])
                              if(len(list(paper_url[archive]["results"].keys()))>0 and item["data"]["callNumber"]=="" ):
                                  toADD["callNumber"]==""
                                  toADD["callNumber"]=json.dumps(paper_url[archive]["results"])
                              else:
                                  print("results >",item["data"]["callNumber"])
                              if(len(list(toADD.keys()))>0):   
                                 print(toADD)
                                 patch_withdata(item["key"],toADD)
                      elif(item["data"]["title"] in paper_title_url.keys()):
                          ...
                          if(len(paper_url[arch_url]["shortname"])>0):
                            toADD["shortTitle"]=", ".join(list(set(paper_url[arch_url]["shortname"])))
                          ...
                          #     print("title >",item["data"]["shortTitle"])
                              #"archiveLocation":datasets_url[archive]["name"]
                          #     toADD["shortTitle"]=dict_papers[url]["shortname"]
                          if(paper_url[arch_url]["code"]!="" and (item["data"]["archiveLocation"] in ["","na","NA"]) ):
                              toADD["archiveLocation"]=paper_url[arch_url]["code"]
                          else:
                              print("CODE >",item["data"]["archiveLocation"])
                        
                          if(len(list(paper_url[arch_url]["results"].keys()))>0 and item["data"]["callNumber"]=="" ):
                              toADD["callNumber"]=json.dumps(paper_url[arch_url]["results"])
