...
def getDOI_ref(title):
    url="https://api.crossref.org/works?rows=2&order=desc&sort=score&mailto="+email+"&select=DOI%2Ctitle&query="
                ...
                while(nb_res>start+100):
                    ...
                    for paper in dict_papers[key]["items"]:
                        if("DOI" in paper["data"].keys()):
                            ...
                            if((paper["data"]["DOI"] in ["","nan","NAN","NA"]) and "/" not in paper["data"]["DOI"]):
                                papers_to_update[paper["key"]]={"title":paper["data"]["title"],"DOI":None}
                
      
            
if getDOIData==True:
  print(">>>>>>>>>",len(list(papers_to_update.keys()))," PAPERS TO UPDATE")
  nb_to_rq=len([k for k in papers_to_update.keys() if papers_to_update[k]["DOI"]==None ])
  print(">>>>",nb_to_rq," PAPERS TO RQ AGAIN")
  n=0
  for k in papers_to_update.keys():
