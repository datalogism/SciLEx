...
surveys = pd.read_csv(collect_dir+'surveys.csv')
models = pd.read_csv(collect_dir+'models.csv')
dataset = pd.read_csv(collect_dir+'datasets.csv')
...
all_data['code'] = str('False')
all_data['bench'] = str('False')
#all_data['title'] = str('none')
all_data["gephi_label"]=""
all_data.reset_index()
nb_papers=all_data.shape[0]
nb_no_doi=len(all_data[all_data.DOI.isnull()])
nb_no_citation=len(all_data[all_data.Extra.str.contains("['", regex=False)==True])
...
for row in all_data2.itertuples():
    ...
    renamed=False
    if(str( getattr(row, "Manual_Tags"))!='nan' and "PWC:havecode" in  getattr(row, "Manual_Tags")):
       #print("code")
       all_data.at[getattr(row, "index_col"),'code']="True"

    if(str( getattr(row, "Manual_Tags"))!='nan'and "PWC:isbenchmarked" in  getattr(row, "Manual_Tags")):
        print("benchmarked")
        
        all_data.at[getattr(row, "index_col"),'bench']="True"
             
    if( str(getattr(row, "Author"))!='nan' and  str(getattr(row, "Author"))[0]==","):
        temp=getattr(row, "Author")[2:len(getattr(row, "Author"))]
        #all_data.at[row.Index, 'Author'] = temp
        
        all_data.at[getattr(row, "index_col"),'Author']=temp
        
        #""
        #"PWC:"
    
    if(str(getattr(row, "Short_Title")) not in ['nan',"","NA"]):
    
        label_t=str(getattr(row, "Short_Title"))
        if("," in str(getattr(row, "Short_Title"))):
            label_t=str(getattr(row, "Short_Title")).split(", ")[0]
        temp=label_t.replace("  ","")
        temp=temp+"_"+str(getattr(row,"Publication_Year"))
        temp=temp.replace(".0","")
        all_data.at[getattr(row, "index_col"),'gephi_label']=temp
        
        
    elif(str(getattr(row, "Author"))  not in ['nan',"","NA"] and str(getattr(row, "Publication_Year")) not in ['nan',"","NA"] ):
        temp=[txt.strip() for txt in str(getattr(row, "Author")).replace(";",",").replace("  ","").split(",") if txt!=" " and txt!=""]
        if(" " in temp[0]):
            temp=temp[0].split(" ")[0]
        else:
            temp=temp[0]
    ...
    if str(row.Extra)!='nan' and "citing" in row.Extra  and  "cited" in row.Extra:
        ...
        for citing in citations["citing"]:
            ...
            if(citing not in DOI_referenced):  
                ...
                temp=all_data[all_data.DOI==citing]
                added=True
                new2={"Id":temp["DOI"].values[0], 
        ...
        for cited in citations["cited"]:
            ...
            if(cited not in DOI_referenced): 
                ...
                temp=all_data[all_data.DOI==cited]
