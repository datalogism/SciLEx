...
for i, row in all_data.iterrows():
    ...
    if(str(row.Author)!='nan' and str(row.Author)!=""   and str(row["Publication Year"])!="nan" and str(row["Publication Year"])!=""):
    ...
    if str(row.Extra)!='nan' and  "['" in row.Extra:
        ...
        for cit in citations:
            ...
            if(cit not in DOI_referenced):       
                new={"Id":cit, "Label":"new_"+str(count_new), "origin":"new paper","Year":str(row["Publication Year"]).replace(".0","")}
                ...
                temp=all_data[all_data.DOI==cit]
                new2={"Id":temp["DOI"].values[0], "Label":temp["gephi_label"].values[0], "origin":str(temp["orig"].values[0]),"Year":str(row["Publication Year"]).replace(".0","")}
    ...
    if( str(row.DOI)!='nan' and str(row.DOI)!=''):
        if(row.DOI not in nodes.Id.explode().unique()):
            ...
            if(row.DOI!="" and row.DOI!="nan" and "/" in row.DOI):
                new={"Id":str(row.DOI), "Label":str(row.gephi_label), "origin":str(row.orig),"Year":str(row["Publication Year"]).replace(".0","")}
                if("dtype" in str(new)):
                    print("3",new)
                nodes=nodes.append(new,ignore_index=True)
nodes["End_Date"]="2023"
nodes.to_csv(collect_dir+'/nodes220323_2.csv', index=False, sep='\t', encoding='utf-8')
