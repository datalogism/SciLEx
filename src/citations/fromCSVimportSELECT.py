...
for index, row in file_new_art.iterrows():
    ...
    if(date > 2018):
        ...
        if(interest==1):
            row["title"]
        tempo={"year":date,"duree":duree,"nb_cit":nb_cit,"ratio":ratio,"interest":interest}
...
if(r_collections.status_code ==  200 ):
    while(continue_==True):
        print('HEY')
        r_items = requests.get(url+"/items?limit=100&start="+str(start), headers=headers)
...
nb=0
for k in doi_stats.keys():
    if(doi_stats[k]["interest"]==1):
        nb+=1

to_add_list=[]
for doi in doi_stats.keys():
