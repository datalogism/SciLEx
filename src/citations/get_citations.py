        ...
        for lib in lib_list:
            ...
            if(r_items.status_code ==  200 ):
                ...
                for paper in dict_papers[key]["items"]:
                    if("DOI" in paper["data"].keys()):
                        
                        paper_to_existing_ref[paper["data"]["DOI"]]=[]
                        if(keep_citation_data and "/" in paper["data"]["DOI"]):
                            if("extra" in  paper["data"].keys() and "citing" in paper["data"]["extra"]):
                                try:
                                    paper_to_existing_ref[paper["data"]["DOI"]]=json.loads(paper["data"]["extra"].replace("'",'"'))
                                except:
                                    print("bad data in extra",paper["data"]["DOI"])
                            else:
                                paper_to_existing_ref[paper["data"]["DOI"]]=[]
                            nb_citation_toget += 1
                        elif(keep_citation_data==False and "/" in paper["data"]["DOI"]):
                            nb_citation_toget += 1
            sleep(3)
        ...
        for paper in items:
            if("DOI" in paper["data"].keys()):
                if("/" in paper["data"]["DOI"]):
                    ...
                    # doi_str=select
                    item_id=paper["key"]
                    if(doi_str in paper_to_existing_ref.keys() and paper_to_existing_ref[doi_str]==[]):
                        ...
                        try:
                             ...
                             if(len(resp_cit)>0):
                                print("HEY")
                                if(doi_str not in citations.keys()):
                                        citations[doi_str]={"citing":[],"cited":[]}
                                        
                                for cit in resp_cit:
                                        citations[doi_str]["citing"].append(cit["citing"])

                                print("-",len(citations[doi_str]["citing"])," citations")
                        except:
...
if pushToZotero==True:
    for ref in citations.keys():
        if(ref in zotid_doi.keys()):
            z_id=zotid_doi[ref]
            list_r=citations[ref]
            if(z_id not in done and len(list_r)>0 and ref not in paper_to_existing_ref.keys() or paper_to_existing_ref[ref]==[]):       
